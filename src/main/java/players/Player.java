...
public class Player implements Runnable {
	...
	/**
	 * queue used to store sent messages
	 */
	protected Queue<String> sentMessageQueue;
	
	/**
	 * queue used to store received messages
	 */
	protected Queue<String> receivedMessageQueue;
	 ...
	 */
	public Player(Queue<String> sentMessageQueue,Queue<String> receivedMessageQueue, ReentrantLock lock) {
	 ...
	 * @return True if sent message queue , received message queue and the lock are not null
	 */
	protected boolean initPlayer() {

		log.info("[{}] started.", Thread.currentThread().getName());
		return (sentMessageQueue != null && receivedMessageQueue != null && lock != null);
	}
	...
	protected void play() {
		...
		try {
			...
			if (numberOfReceivedMessages < MAX_NUMBER_RECEIVED_MESSAGES && !receivedMessageQueue.isEmpty()) {
				String receivedMessage = receivedMessageQueue.poll();
				numberOfReceivedMessages++;
				log.info("[{}] received {}.", Thread.currentThread().getName(), receivedMessage);
				receivedMessageBuilder.append(receivedMessage).append(" ");
			}

			if (receivedMessageBuilder.length() > 0 && numberOfSentMessages < MAX_NUMBER_SENT_MESSAGES && sentMessageQueue.isEmpty()) {
